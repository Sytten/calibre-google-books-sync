...
class DemoDialog(QDialog):
    ...
    def marked(self):
        ...
        db = self.db.new_api
        matched_ids = {
            book_id for book_id in db.all_book_ids() if len(db.formats(book_id)) == 1
        }
    ...
    def view(self):
        ...
        db = self.db.new_api
        for book_id, timestamp in db.all_field_for(
            "timestamp", db.all_book_ids()
        ).items():
    ...
    def update_metadata(self):
        ...
        if not rows or len(rows) == 0:
            return error_dialog(
                self.gui, "Cannot update metadata", "No books selected", show=True
            )
